 ...
 * iter == x.end()일 떄 iter = x.begin()으로 계속해서 업데이트
 ...
 * 
 * - 생각
 * 출력 포맷이 해당 문제와 같이 <%d, %d, %d, %d, .... %d>
 * 반복문으로 구성할 수 있는 출력 포맷과 마지막 출력 포맷의
 * 형태가 다를 경우 n-1만큼 출력하고 마지막 n번째 출력은
 * 따로하는 것이 편함.
 * 
 * SOLVE 1> 
 * 처음 문제를 풀 때 q.push(q.front())의 부분에서 queue가
 * 비어있지 않은 상황에서만 해당 코드가 동작하도록 조건을
 * 지정해주었으나, 이러한 경우 제출 시 에러가 발생한다.
 * 자세한 에러 원인에 대해서는 추후에 더 고민해봐야 할 듯하다.
 * 
*/
