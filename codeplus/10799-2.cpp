 ...
 * 
 * - 생각
 * 레이저와 파이프의 끝을 어떻게 구분하는지가 중요한 문제
 * 레이저는 바로 이전에 push 한 값이 '('인 것이고,
 * 파이프는 스택의 top이 '('인 것 중 레이저의 경우를 제외한 것
 * 
 * SOLVE 2> 
 * 스택에 저장되는 값은 항상 '('이고, ')'는 스택에 저장하지 않고
 * 이전까지 스택에 저장한 값을 통해 적절한 처리를 해 주는 부분임.
 * 이는 결국 스택에 저장하는 자료는 '('라는 문자를 굳이 넣어 줄 필요가
 * 없고 레이저와 파이프의 끝을 구분할 수있는 자료를 정해서 사용해도
 * 된다.
 * 
 * 이를 구현하는 방법으로 현재 처리번호를 스택에 push하는 방법을
 * 사용하면 레이저와 파이프를 구분하는 방법 또한 쉽게 구현이 가능할 것이다.
 * 
 * 
*/
...
int main()
{
    ...
    while(cin >> ch){
        ...
        else{ //s.empty()는 고려할 필요가 없음(문제 조건 상)
            // 레이저
            if(s.top()+1 == i){
                s.pop();
                sum += s.size();
